...
const voicedConsonantArray = ["z","d","n","m","r","l","ch","g","v","b"];
const specialVoicedConsonantArray = ["z","d","h","y","tr","w","ch","g","v","b"];
//♥️', '♦️', '♣️', '♠️
const getBen4Phonetics = (text) => {
    ...
    function getSyllableFromRule(ruleObj,thisNum) {
        ...
        switch (ruleObj.type) {
            case 0:
                    thisSyllable = ruleObj.cluster + benVowels[thisNum.charAt(2)] + majorValues[thisNum.charAt(3)];                
                break;
            case 1:
                thisSyllable = majorValues[thisNum.charAt(2)] + ruleObj.cluster + majorValues[thisNum.charAt(3)];
                break;
            case 2:
                if (ruleObj.shortVowelVersion && (benVowels[thisNum.charAt(3)].length == 1)) {
                    thisSyllable = majorValues[thisNum.charAt(2)] + benVowels[thisNum.charAt(3)] + ruleObj.cluster[0] + ruleObj.cluster;
                } else {
                    thisSyllable = majorValues[thisNum.charAt(2)] + benVowels[thisNum.charAt(3)] + ruleObj.cluster;
                }
                break;
            case 3:
                var finalCons = majorValues[thisNum.charAt(3)];
                if (finalCons == "s" || finalCons == "t" || finalCons == "sh" || finalCons == "k" || finalCons == "f" || finalCons == "p") {
                        thisSyllable = ruleObj.cluster + benVowels[thisNum.charAt(2)] + voicedConsonantArray[thisNum.charAt(3)];
                } else {
                        thisSyllable = ruleObj.cluster + benVowels[thisNum.charAt(2)] + 'r' + rModifiedConsonantArray[thisNum.charAt(3)];
                }
                break;
            case 4:
                thisSyllable = majorValues[thisNum.charAt(1)] + benVowels[thisNum.charAt(2)] + 'r' + rModifiedConsonantArray[thisNum.charAt(3)];
                break;
            case 5:
                thisSyllable = ruleObj.cluster + majorValues[thisNum.charAt(2)] + benVowels[thisNum.charAt(3)];
                break;
             case 6:
                thisSyllable = specialVoicedConsonantArray[thisNum.charAt(2)] + benVowels[thisNum.charAt(3)];
                break;
            case 7:
                 thisSyllable = benVowels[thisNum.charAt(2)] + "r" + rModifiedConsonantArray[thisNum.charAt(3)];
                    break;
        }
        return thisSyllable;
    }
...
const getCardParts = (text) => {
    ...
    const cardParts = {'cardVal1' : convertProcessableStringToCardText(processableString[0]), 'cardVal2' : convertProcessableStringToCardText(processableString[2]), 'suit1' : convertProcessableStringToCardText(processableString[1]), 'suit2': convertProcessableStringToCardText(processableString[3])}
      ...
      //get the array that we want to populate the image set with
        ...
        switch(phoneticsType) {          
            case 'maj':
                if (setType === '2d') {
                    for (let i = 0; i < 100; i++) {
                        const twoDigitPhonetics = majorValues[i.toString().padStart(2, '0')[0]] + "+" + majorValues[i.toString().padStart(2, '0')[1]];
                        phoneticsArray.push(twoDigitPhonetics);
                    }
                }
                if (setType === '3d') {
                    for (let i = 0; i < 1000; i++) {
                        const threeDigitPhonetics = majorValues[i.toString().padStart(3, '0')[0]] + "+" + majorValues[i.toString().padStart(3, '0')[1]] + "+" + majorValues[i.toString().padStart(3, '0')[2]];
                        phoneticsArray.push(threeDigitPhonetics);      
                    }          
                }
                if (setType === '4d') {
                    for (let i = 0; i < 10000; i++) {
                        const fourDigitPhonetics = majorValues[i.toString().padStart(4, '0')[0]] + "+" + majorValues[i.toString().padStart(4, '0')[1]] + "+" + majorValues[i.toString().padStart(4, '0')[2]]+ majorValues[i.toString().padStart(4, '0')[3]];
            ...
            case 'ben':
                if (setType === '3d') {
                    for (let i = 0; i < 1000; i++) {
                        const threeDigitPhonetics = majorValues[i.toString().padStart(3, '0')[0]] + benVowels[i.toString().padStart(3, '0')[1]] + majorValues[i.toString().padStart(3, '0')[2]];
                        phoneticsArray.push(threeDigitPhonetics);   
                    }             
                }
                // if (setType === '4d') {
            ...
            case 'kben':
                if (setType === '4d') {      
                    for (let i = 0; i < 10000; i++) {                         
                        const fourDigitPhonetics = getBen4Phonetics(i.toString().padStart(4, '0')); //Do we need the toString?
                        phoneticsArray.push(fourDigitPhonetics);        
                    }                        
                }
                if (setType === '2c') {
