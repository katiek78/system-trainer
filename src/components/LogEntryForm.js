...
const LogEntryForm = ({
  ...
  const [form, setForm] = useState({
    entryDate: forNewEntry
      ? getTodayDate()
      : formatDate(logEntryForm.entryDate),
  ...
  return (
    <>
      ...
      >
        ...
        <input
          className="shadow appearance-none border rounded w-20 ml-3 mr-10 mt-1 mb-2 py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
          type="text"
          maxLength="10"
          name="score"
          value={form.score}
        ...
        {(ML_DISCIPLINES.includes(form.discipline) ||
          form.discipline === "Speed Cards") && (
          <>
        ...
        >
          {journeys &&
            sortedJourneys.map((j) => <option value={j._id}>{j.name}</option>)}
          {publicJourneys &&
            sortedPublicJourneys.map((j) => (
