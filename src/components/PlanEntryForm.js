...
const PlanEntryForm = ({ formId, planEntryForm, forNewEntry = true }) => {
  ...
  const putData = async (form) => {
    ...
    try {
      ...
      mutate(`/api/planEntries/${id}`, data, false);
      router.push(`/plan`);
    } catch (error) {
  ...
  const handleChange = (e) => {
    ...
    setForm({
      ...form,
      [name]: value,
      frequencySpecifics: resetFrequencySpecifics
        ? []
        : form.frequencySpecifics,
    });
  };
  ...
  const handleAddSpecifics = () => {
    ...
    const validatedEntries = specificsArray.flatMap((entry) => {
      ...
      if (form.frequencyType === "W") {
        ...
        return !isNaN(numericEntry) && numericEntry >= 1 && numericEntry <= 31
          ? [numericEntry]
          : [];
      }
  ...
  const handleSubmit = (e) => {
    ...
    if (
      updatedFormData.frequencySpecifics.length !== updatedFormData.frequency
    ) {
      updatedFormData = {
        ...updatedFormData,
        frequency: form.frequencySpecifics.length,
      };
  ...
  return (
    <>
      ...
      >
        ...
        <input
          className="shadow appearance-none border rounded w-14 ml-3 mr-1 mt-1 mb-2 py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
          type="text"
          maxLength="4"
          name="frequency"
        ...
        times per
        <select
          className="shadow appearance-none border rounded w-20 ml-3 mt-1 mb-2 py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
          name="frequencyType"
        ...
        {form.frequencyType !== "D" && (
          <>
            <br />
            <label htmlFor="frequencySpecificsInput">
              Which days do you want to do this?
            </label>
            <input
              className="shadow appearance-none border rounded w-20 ml-3 mr-3 mt-1 mb-2 py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
              type="text"
              maxLength="50"
