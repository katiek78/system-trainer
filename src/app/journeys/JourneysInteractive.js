...
export default function JourneysInteractive({
  ...
  publicJourneys: initialPublicJourneys,
  user,
}) {
  ...
  const handleJourneyView = () => setIsJourneyView(true);
  const handleAssignmentView = () => setIsJourneyView(false);
  ...
  const moveSelectedJourney = (idx, dir) => {
    setSelectedJourneyIds((prev) => {
      const arr = [...prev];
      if (dir === "up" && idx > 0)
  ...
  const handleMoveToFolder = async (journeyId, folderId) => {
    try {
      ...
      setJourneys(
        journeys.map((j) => (j._id === journeyId ? { ...j, folderId } : j))
      );
    } catch (error) {
  ...
  const getAssignmentsForDiscipline = (d) => {
    ...
    if (matchingAssignments.length === 0) return <p></p>;
    return (
      <div>
              ...
              <div key={i} className="mb-1">
                ...
                <span className="inline-flex items-center space-x-2 px-4 py-2 bg-gradient-to-r from-blue-700 to-blue-800 rounded-xl text-white font-medium text-lg shadow-sm border border-blue-900 mr-2">
                  {set.journeyIDs.map((id, j) => (
                    <span key={id}>
                      <span onClick={() => handleClickJourney(id)}>
            ...
            <div className="bg-white dark:bg-slate-800 py-5 px-5 rounded">
              ...
              {folders.length > 0 &&
                sortedFolders.map((folder) => {
                  const journeysInFolder = journeys
                    .filter((j) => j.folderId === folder._id)
            ...
            <div className="bg-white dark:bg-slate-800 py-5 px-5 rounded">
              ...
              {publicJourneys.length > 0 &&
                sortedPublicJourneys.map((journey) => (
                  <p className="font-semibold" key={journey._id}>
                    <Link href={`/journeys/${journey._id}/`}>
                      {journey.name}
                    </Link>{" "}
                    <FontAwesomeIcon
                  ...
                  {[...ML_DISCIPLINES, ...TRADITIONAL_DISCIPLINES].map(
                    (discipline) => (
                      ...
                      >
                        ...
                        <div className="sm:table-cell border sm:border border-gray-400 px-1 sm:px-4 py-2">
                          <Link
                            href={`/disciplines/${discipline}/editAssignment`}
                          >
                            <FontAwesomeIcon
