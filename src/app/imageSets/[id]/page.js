...
export default function ImageSetPage() {
  const router = useRouter();
  const params = useParams();
  const imageSetID = params.id;

  const [allNames, setAllNames] = useState({ images: [] });
  const [imageSets, setImageSets] = useState([]);
  const [isPublicImageSet, setIsPublicImageSet] = useState(false);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        // Fetch the imageSet metadata (names only)
        const namesRes = await fetch(`/api/imageSets/getName?id=${imageSetID}`);
        if (namesRes.ok) {
          const namesData = await namesRes.json();
          setAllNames(namesData);
          setIsPublicImageSet(!namesData.userId);
        }

        // Fetch all image set names for import dropdown
        const setsRes = await fetch("/api/imageSets/names");
        if (setsRes.ok) {
          const setsData = await setsRes.json();
          setImageSets(setsData);
        }
      } catch (error) {
  ...
  return (
    <>
      <div className="z-10 justify-between w-full font-mono pl-2 md:pl-2 lg:pl-0">
        ...
        {!isShowingPhoneticsDiv && (!isPublicImageSet || isAdminUser) && (
          <button
        ...
        {!isShowingImportPhoneticsDiv && (!isPublicImageSet || isAdminUser) && (
          <button
        ...
        {(!isPublicImageSet || isAdminUser) && (
          <Link href={`/imageSets/${imageSetID}/pasteImages`}>
