  ...
  return (
    <>
      <div className="z-10 justify-between font-mono text-lg max-w-5xl w-full ">
        ...
        {
          system.imageSets.length > 0 &&
            includedImageSetNames.map((el) => <p>{el}</p>) //Eventually you want delete button, move up/down buttons next to each set and the option to select another set to include
        }

        {system.imageSets.length === 0 && imageSets.length > 0 && (
          <p>
            You have not included any image sets in your system. Select an image
            set or sets to include - e.g. for PAO, select your P, then A, then O
            system. For a 3-digit system where you have two images per locus,
            select your 3-digit set twice.
          </p>
        )}
        <br />
        {
          imageSets.length > 0 && imageSets.map((el) => <p>{el.name}</p>) //Eventually you want delete button, move up/down buttons next to each set and the option to select another set to include
        }

        {imageSets.length === 0 && (
          <>
            <p>
              You haven't created any image sets yet. Create your first image
              set and then you can include it in a system.
            </p>
            <Link href="/newImageSet" as={`/newImageSet`}>
