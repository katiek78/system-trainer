  ...
  const handleCopyPublic = async (id) => {
    try {
      ...
      if (!publicImageSetResponse.ok) {
        throw new Error(
          publicImageSetResponse.status + " when fetching image set"
        );
      }
            ...
            publicImageSets.map((imageSet) => (
              <p className="font-semibold">
                ...
                <FontAwesomeIcon
                  ...
                  icon={faCopy}
                  size="1x"
                  onClick={() => handleCopyPublic2(imageSet._id)}
                />
              </p>
            ))}
        </div>
...
export const getServerSideProps = withPageAuthRequired({
  getServerSideProps: async ({ req, res }) => {
    ...
    //only want to send the name and ID of private image sets belonging to this user
    ...
    const publicImageSetResult = await ImageSet.find(
